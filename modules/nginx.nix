    ...
    in
    {
      ...
      options.services.nginx.headersNoAccessControlOrigin =
        ...
        mkConst ''
          ...
          add_header Content-Security-Policy "script-src 'self' 'unsafe-inline' 'unsafe-eval' ${domain} *.${domain}; object-src 'self' ${domain} *.${domain}; img-src 'self' ${domain} *.${domain}; data: https:; base-uri 'self'; frame-ancestors 'self';" always;
