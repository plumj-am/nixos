...
const generatorInfo =
	"Generated by mcporter@0.0.0 — https://github.com/steipete/mcporter"
...
const forceEnableColor = FORCE_COLOR === "1" || FORCE_COLOR === "true" ||
	FORCE_COLOR === "2" || FORCE_COLOR === "3"
const hasNoColor = process.env.NO_COLOR !== undefined
const stdoutStream = process.stdout as NodeJS.WriteStream | undefined
const supportsAnsiColor = !hasNoColor &&
...
function configureGeneratedCommandHelp(command: Command): void {
	command.configureHelp({
		commandUsage(target) {
			const usage = (target.name() + " " + target.usage()).trim() ||
				target.name()
...
function renderStandaloneHelp(): string {
	const colorfulTitle = tint.bold(embeddedName) + " " +
		tint.dim("— " + embeddedDescription)
...
function formatGlobalFlags(): string {
	...
	const formatted = entries.map(([flag, summary]) =>
		"  " + flag.padEnd(28) + summary
	)
...
function formatQuickStart(): string {
	...
	const formatted = examples.map(([cmd, note]) =>
		"  " + cmd + "\n    " + tint.dim("# " + note)
	)
	return [header, ...formatted].join("\n")
}
