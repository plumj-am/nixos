...
in
{
  programs.nvf.settings.vim.autocmds = [
    {
      ...
      event    = [ "InsertEnter" ];
      callback = mkLuaInline ''function() vim.fn.clearmatches() end'';
    }
    {
      desc     = "highlight trailing whitespace on insert leave";
      event    = [ "InsertLeave" ];
      callback = mkLuaInline ''function() vim.fn.matchadd("ws", [[\(\s\|\r\)\+$]]) end'';
    }
    {
      desc     = "highlight yanked text";
      event    = [ "TextYankPost" ];
      callback = mkLuaInline ''function() vim.highlight.on_yank({ higroup = "Visual", timeout = 500 }) end'';
    }
    {
      desc     = "auto-resize windows on vim resize";
      event    = [ "VimResized" ];
      pattern  = [ "*" ];
      callback = mkLuaInline ''function() vim.cmd("wincmd =") end'';
    }
    {
  ...
  programs.nvf.settings.vim.luaConfigRC.whitespace = "
