    ...
    virtualHosts.${fqdn} = merge config.services.nginx.sslTemplate {
      ...
      locations."/" = {
        ...
        extraConfig = ''
          ...
          add_header Content-Security-Policy "script-src 'self' 'unsafe-inline' 'unsafe-eval' ${domain} *.${domain} kit.fontawesome.com; img-src 'self' data: https: ghchart.rshah.org;" always;
